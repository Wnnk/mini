...
export const useLasso = (stage: Konva.Stage, layer: Konva.Layer) => {
  ...
  const mousedown = () => {
    ...
    if (appStore.selectArea) {
      appStore.selectArea.destroy();
    ...
    line = new Konva.Line({
      ...
      stroke: "black",
      strokeWidth: 1,
      name: `lasso_${Math.floor(Math.random() * 10000)}`,
      tension: 0.3,
    });
    layer.add(line);
  };
